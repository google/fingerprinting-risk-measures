...
* **Execution Time:** Approximately 30 human-minutes to set up the environment and execute the notebook cells, plus about 100 compute-minutes to run the simulations and plot the full results. (Note: The statistical significance requires multiple runs per data point. The compute time can be lowered by reducing the `trials` variable within the notebooks, at the expense of higher variance).  
...
- **Time:** \~5 human-minute \+ \~15 compute-minutes.  
