...
* **Execution Time:** Approximately 25 human-minutes to set up the environment and execute the notebook cells, plus about 1 compute-hour to run the simulations and plot the full results. (Note: The statistical significance requires multiple runs per data point. The compute time can be lowered by reducing the `trials` variable within the notebooks, at the expense of higher variance).  
...
- **Expected Result:** Generates simulated plots matching the figures in Section 4.1 of the paper, illustrating how sample size and domain size affect Shannon and normalized entropy estimates.
...
- **Expected Result:** Generates plots illustrating the case study results for Shannon and normalized entropy on the real-world browser attribute dataset.
...
- **Expected Result:** Computes and plots the comparison between Tsallis entropy, Shannon entropy, and normalized entropy using the empirical dataset, matching the figures in Section 6.2.
...
- **Expected Result:** Evaluates the Chao-Shen, Grassberger, and James-Stein estimators alongside Tsallis entropy. This reproduces the estimation error analysis found in Section 6.3
...
- **Expected Result:** Generates the unicity comparison plots presented in Appendix F of the paper, showing how Shannon entropy and PYP are better suited for estimating uniqueness, but Tsallis entropy is better at estimating fingerprinting risk for cases in which users do not have unique values.
...
This artifact provides a general framework for assessing fingerprinting risk measures. The Jupyter notebooks can be easily adapted to compute entropy metrics (Shannon, normalized, and Tsallis) for other datasets by replacing the target CSV file and adjusting the DataFrame column loading logic. Furthermore, researchers investigating other properties of tracking attributes could reuse the implementation of the advanced entropy estimators and the unicity mapping logic for broader device fingerprinting studies.  
